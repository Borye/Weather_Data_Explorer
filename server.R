...
shinyServer(function(input, output){
    ...
    weather.sub <- reactive({
        # Sys.setlocale()
        data <- subset(weather, 
               t_json >= as.numeric(as.POSIXlt(paste("2015-01-13", input$range_1), format = "%Y-%m-%d %H:%M", origin = "2015-01-13 00:00:00", tz = "Asia/Taipei"))
               & t_json <= as.numeric(as.POSIXlt(paste("2015-01-13", input$range_2), format = "%Y-%m-%d %H:%M", origin = "2015-01-13 00:00:00", tz = "Asia/Taipei")) 
